...
public:
    ...
    void setup(int size) override {
        ...
        // JUCE uses interleaved format [r0,i0,r1,i1,...] and needs 2*size for real FFT
    ...
    void forward(const float* in, std::complex<float>* out) override {
        ...
        std::fill(interleaved_buffer_.begin() + size_, interleaved_buffer_.end(), 0.0f);
        ...
        for (int i = 1; i < size_/2; ++i) {
            out[i] = std::complex<float>(
                interleaved_buffer_[i * 2],      // real
                interleaved_buffer_[i * 2 + 1]   // imag
            );
        }
        
        out[size_/2] = std::complex<float>(interleaved_buffer_[1], 0.0f);  // Nyquist
    }
    ...
    void backward(const std::complex<float>* in, float* out) override {
        ...
        // JUCE doesn't normalize, so we need to divide by size
        for (int i = 0; i < size_; ++i) {
            out[i] = interleaved_buffer_[i] / size_;
        }
    }
