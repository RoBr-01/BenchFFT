...
class FFTWBackend : public FFTBackend {
   public:
    ...
    void setup(int size) override {
        ...
        complex_out_ =
            reinterpret_cast<std::complex<float>*>(fftwf_alloc_complex(size));
        complex_in_ =
            reinterpret_cast<std::complex<float>*>(fftwf_alloc_complex(size));
        real_out_ = fftwf_alloc_real(size);

        // FFTW_ESTIMATE: fast plan creation, good SIMD plans on modern CPUs.
        // FFTW_MEASURE would find a marginally faster plan but takes seconds
        // for large sizes and overwrites the input/output buffers during
        // planning, which interferes with benchmark setup.
    ...
    void cleanup() override {
        ...
        if (plan_forward_c2c_)
            fftwf_destroy_plan(plan_forward_c2c_);
        if (plan_backward_c2c_)
            fftwf_destroy_plan(plan_backward_c2c_);
        if (real_in_)
            fftwf_free(real_in_);
        if (complex_out_)
            fftwf_free(complex_out_);
        if (complex_in_)
            fftwf_free(complex_in_);
        if (real_out_)
            fftwf_free(real_out_);
