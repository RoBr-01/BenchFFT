...
public:
    ...
    void forward(const float* in, std::complex<float>* out) override {
        ...
        plan_real_->execute(kfr_out, kfr::make_univector(real_buffer_.data(), size_), 
                           temp_buffer_.data());
    ...
    void forward_complex(const std::complex<float>* in, 
        ...
        plan_complex_->execute(kfr_out, kfr_in, temp_buffer_complex_.data(), kfr::cfalse);
    ...
    void backward_complex(const std::complex<float>* in, 
        ...
        kfr::univector<kfr::complex<float>> kfr_out(size_);
        plan_complex_->execute(kfr_out, kfr_in, temp_buffer_complex_.data(), kfr::ctrue);
    ...
    std::string name() const override { return "KFR"; }
