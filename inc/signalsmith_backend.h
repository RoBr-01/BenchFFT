...
public:
    ...
    void forward_complex(const std::complex<float>* in, 
        ...
        // Copy input
        std::vector<std::complex<float>> input_copy(in, in + size_);
        
        // Perform complex FFT using FFT class
        complex_fft_.fft(input_copy.data(), complex_buffer_.data());
    ...
    void backward_complex(const std::complex<float>* in, 
        ...
        complex_fft_.ifft(input_copy.data(), complex_buffer_.data());
        
        // Signalsmith's ifft() does NOT normalize, so we must divide by size
        for (int i = 0; i < size_; ++i) {
            out[i] = complex_buffer_[i] / static_cast<float>(size_);
        }
    }
